...
class CategoryService extends Service {
  ...
  async getIncluded() {
    ...
    // 查找所有分类及其子类的数目
    const result = await this.app.mysql.query('SELECT p.id,p.title,count(s.title) AS children_count,p.parent_id FROM fe_level AS p LEFT JOIN fe_level AS s ON s.parent_id = p.id GROUP BY p.id ORDER BY p.id;');
